...
###    sudo iptables -R INPUT ЦИФРА -p tcp --dport ПОРТ-SSH -s "$dns1" -j ACCEPT
###    sudo iptables -R INPUT ЦИФРА -p tcp --dport 80 -s "$dns1" -j ACCEPT
###    sudo iptables -R INPUT ЦИФРА -p tcp --dport 443 -s "$dns1" -j ACCEPT
###    sudo iptables -R INPUT ЦИФРА -p tcp --dport ПОРТ-ДЛЯ-ПРОКСИ -s "$dns1" -j ACCEPT
###    sudo iptables -R INPUT ЦИФРА -p tcp --dport ПОРТ-ДЛЯ-ПРОКСИ+ВЕБ -s "$dns1" -j ACCEPT
###fi
###}
###logika1
...
###    sudo iptables -R INPUT ЦИФРА -p tcp -dport ПОРТ-SSH -s "$dns2" -j ACCEPT
###    sudo iptables -R INPUT ЦИФРА -p tcp -dport 80 -s "$dns2" -j ACCEPT
###    sudo iptables -R INPUT ЦИФРА -p tcp -dport 443 -s "$dns2" -j ACCEPT
###    sudo iptables -R INPUT ЦИФРА -p tcp -dport ПОРТ-ДЛЯ-ПРОКСИ -s "$dns2" -j ACCEPT
###    sudo iptables -R INPUT ЦИФРА -p tcp -dport ПОРТ-ДЛЯ-ПРОКСИ+ВЕБ -s "$dns2" -j ACCEPT
###fi
###}
###logika2
